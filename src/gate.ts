...
const KNOWN_GATES: Record<KnownReasonBase, (ctx: GateContext) => Gate> = {
  ...
  merge_onto_trunk: ({ reason, taskId }) =>
    ...
    ], [STOP], 'Switch the checkout back to the work branch (git switch devloop/<goal>), then resume (恢复循环 on the page, or devloop resume); the task merges on the next tick.'),
  ...
  merge_detached_head: ({ reason, taskId }) =>
    ...
    ], [STOP], 'Check out the work branch (git switch devloop/<goal>), then resume (恢复循环 on the page, or devloop resume); the task merges on the next tick.'),
