...
**当前分支实现 0.2.5（T3 CLI adapters）。0.2.4 在 stacked PR #11。0.3 未开始。**
...
| 0.2.4 | 机械 `/merge` 脚本 | 无 Review PASS 不能合；合完删 worktree | **This stack (PR #11)** |
| 0.2.5 | Codex / Claude CLI 作为 T3 最小接线 | 可选 adapter；生产默认仍是 `noop` / `dsh`，不经过 `RecordingBackend` | **This slice** |
