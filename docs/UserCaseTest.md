...
| UC-14 | Loop | After each tick (including a latched follow-up), `.devloop/PROGRESS.md` exists and `Updated:` advances; optional cost signals fold into usage | Plan 0.3 in `tests/progress.spec.ts`, `tests/service.spec.ts`, `tests/budget.spec.ts` |

## Manual after GitHub publish

1. `pnpm install && pnpm test && pnpm build`
2. After GitHub Release `v0.3.0` exists, install as in [`Install.md`](./Install.md) (`'github:jhfnetboy/DevLoop#v0.3.0'` — quote it on zsh — or the Release tarball). Until then, `github:jhfnetboy/DevLoop` tracks `main` (or this PR branch). `v0.2.3` does not accept `agentBackend: claude` / `codex`.
