...
- T3 CLIs refuse to run at the workspace root (null cwd **or** cwd equal to the workspace). `plan` uses a reserved `_loop-plan` worktree plus read-only / plan permission flags. That id is outside the user task-token alphabet. Plan stdout is copied to workspace `.devloop/PLAN.md`.
