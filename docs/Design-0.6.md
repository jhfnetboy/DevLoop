...
状态：**草案 v2**（2026-09-12），已写入用户 2026-09-12 的决定；第 7 节剩下的一个问题确认后开工。
...
| 版本 | 内容 | 用户能看到什么 |
|---|---|---|
| **0.6.0** | 盘点：「仓库状态」面板 + dry-run 清理 + 执行清理（只 `branch -d`）；PR-LOG 骨架 | 项目页一眼看到分支、worktree、可清理项，点一下清掉已合并分支 |
| **0.6.1** | 大小预算：任务完成后统计 diff，超限停下要求拆分（`task_over_budget`）；规划器提示词写入上限 | 超大的任务不会被提交，会停下说「拆一下」 |
...
依赖关系：0.6.2 等 PR-daemon 规则合进 main；0.6.3 依赖 0.6.1、0.6.2；0.6.4 依赖 0.6.1（规划要按上限拆任务）。0.6.0 可以马上开工。

## 7. 还需要你确认

1. **目录规则**（2026-09-12 待定）：计算「≤2 个顶层目录」时，`tests/`、`docs/` 这类目录算不算。
