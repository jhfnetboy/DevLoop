...
```bash
pnpm pack
tar -tzf jhfnetboy-dsh-devloop-0.6.1.tgz
rm -f jhfnetboy-dsh-devloop-0.6.1.tgz
```
...
Do this immediately after the version bump is on `main`, in the same sitting, so Install.md’s `#v0.6.1` commands are not a 404:

```bash
git checkout main
git pull --ff-only origin main
git tag -a v0.6.1 -m "dsh-devloop 0.6.1"
git push origin v0.6.1
pnpm pack
gh release create v0.6.1 \
  --title "0.6.1" \
  --notes-file docs/Release.md \
  jhfnetboy-dsh-devloop-0.6.1.tgz
```

Pin installs to `'github:jhfnetboy/DevLoop#v0.6.1'` (quotes required on zsh). Attach the `.tgz` so operators can skip git `prepare` / build approval.

## npm registry (when logged in)

Only when `npm whoami` succeeds, `HEAD` **is** the release tag (`git rev-parse HEAD` equals `git rev-parse v0.6.1`), the tree is clean, and `pnpm test` is green.
...
Publish the **inspected tarball** from `main` while it still points at the tag commit (do not `git checkout v0.6.1`: detached HEAD makes pnpm 10.6.3 fail with `ERR_PNPM_GIT_UNKNOWN_BRANCH`):

```bash
git checkout main
git pull --ff-only origin main
test "$(git rev-parse HEAD)" = "$(git rev-parse v0.6.1^{commit})"
pnpm pack
pnpm publish ./jhfnetboy-dsh-devloop-0.6.1.tgz --access public
```

This package is scoped and published with `--access public` (set in `publishConfig`); the tarball is public. Then operators can `dsh plugin --profile web add @jhfnetboy/dsh-devloop@0.6.1` with no git `prepare`.
