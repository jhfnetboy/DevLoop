...
```bash
pnpm pack
tar -tzf dsh-devloop-0.3.0.tgz
rm -f dsh-devloop-0.3.0.tgz
```
...
Do this immediately after the version bump is on `main`, in the same sitting, so Install.md’s `#v0.3.0` commands are not a 404:

```bash
git checkout main
git pull --ff-only origin main
git tag -a v0.3.0 -m "dsh-devloop 0.3.0"
git push origin v0.3.0
pnpm pack
gh release create v0.3.0 \
  --title "0.3.0" \
  --notes-file docs/Release.md \
  dsh-devloop-0.3.0.tgz
```

Pin installs to `'github:jhfnetboy/DevLoop#v0.3.0'` (quotes required on zsh). Attach the `.tgz` so operators can skip git `prepare` / build approval.

## npm registry (when logged in)

Only when `npm whoami` succeeds, `HEAD` **is** the release tag (`git rev-parse HEAD` equals `git rev-parse v0.3.0`), the tree is clean, and `pnpm test` is green.

Publish the **inspected tarball** from `main` while it still points at the tag commit (do not `git checkout v0.3.0`: detached HEAD makes pnpm 10.6.3 fail with `ERR_PNPM_GIT_UNKNOWN_BRANCH`):
...
pnpm pack
pnpm publish ./dsh-devloop-0.3.0.tgz --access public
```

This package is unscoped; the tarball is public. Then operators can `dsh plugin --profile web add dsh-devloop@0.3.0` with no git `prepare`.
