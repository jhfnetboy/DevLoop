...
- 共享 `defaultRunner`：Unix 进程组 SIGTERM/SIGKILL，且 abort 等到 SIGKILL 后再 settle；Windows 经 `cmd.exe` 跑 `.cmd` shim（参数用 cmd 的 `""` 转义，防止 `&` 注入），并用 `taskkill /T` 杀进程树（`spawn` 挂 `error` 监听，避免 ENOENT 打崩宿主）；超 `maxBuffer` 后停写并 destroy stdout/stderr
