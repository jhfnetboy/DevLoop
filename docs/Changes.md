...
- 共享 `defaultRunner`：Unix 进程组 SIGTERM/SIGKILL，且 abort 等到 SIGKILL 后再 settle；Windows 经 `cmd.exe /s /c` 跑 `.cmd` shim（整段命令再包一层引号，token 用 `""` 转义），并用 `taskkill /T` 杀进程树（`spawn` 挂 `error` 监听，避免 ENOENT 打崩宿主）；超 `maxBuffer` 后停写并 destroy stdout/stderr
- T3 `plan` 在保留 worktree `_loop-plan` 里以 **detached HEAD** 跑（不创建、不 reset、不删除 `devloop/_loop-plan` 分支），并拷入 `GOAL.md`；复用前拒绝 symlink/junction 别名，且 git 注册路径必须就是 `_loop-plan`；CLI plan stdout 写入 `.devloop/PLAN.md`，review stdout 写入 `.devloop/REVIEW.md`
