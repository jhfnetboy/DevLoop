...
- 共享 `defaultRunner`：Unix 进程组 SIGTERM/SIGKILL，且 abort 等到 SIGKILL 后再 settle（即使子进程还没 close 也在宽限期结束时 reject，避免 Windows `taskkill` 失败时 promise 挂死）；Windows 经 `cmd.exe /s /c` 跑 `.cmd` shim（整段命令再包一层引号，token 用 `""` 转义，并对 CRT 会吞掉的尾部/`"` 前反斜杠加倍），并用 `taskkill /T` 加 `child.kill` 杀进程树（`spawn` 挂 `error` 监听，避免 ENOENT 打崩宿主）；超 `maxBuffer` 后停写并 destroy stdout/stderr
