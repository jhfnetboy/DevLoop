...
- 共享 `defaultRunner`：Unix 进程组 SIGTERM/SIGKILL，且 abort 等到 SIGKILL 后再 settle；Windows 经 `cmd.exe` 跑 `.cmd` shim，并用 `taskkill /T` 杀进程树；超 `maxBuffer` 后停写并 destroy stdout/stderr
