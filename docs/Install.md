...
Pinned `#v0.6.8` commands and the Release tarball link need the GitHub Release created **after** the version bump is on `main` (see Deploy.md). Until that Release exists, install from current `main` (moving branch, not a pin):
...
```bash
dsh plugin --profile web add 'github:jhfnetboy/DevLoop#v0.6.8'
```
...
After the [v0.6.8 Release](https://github.com/jhfnetboy/DevLoop/releases/tag/v0.6.8) exists, download `jhfnetboy-dsh-devloop-0.6.8.tgz`, then:

```bash
dsh plugin --profile web add ./jhfnetboy-dsh-devloop-0.6.8.tgz
```
...
```bash
dsh plugin --profile web add @jhfnetboy/dsh-devloop@0.6.8
```
...
cd DevLoop
git checkout v0.6.8   # or main, until the tag exists
pnpm install
pnpm test
pnpm build
...
Or fetch the template without a clone (use `main` instead of `v0.6.8` until the tag exists):

```bash
mkdir -p /path/to/your/project/.devloop
curl -fsSL https://raw.githubusercontent.com/jhfnetboy/DevLoop/v0.6.8/templates/GOAL.md \
