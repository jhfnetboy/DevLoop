...
**代码中的体现**：`DevloopService.tick` 写完 STATE 后，对 plan / delegate / review 调用 `AgentBackend.run`（默认 NoopBackend；`agentBackend: 'dsh' | 'claude' | 'codex'` 时 spawn 对应 CLI）。`merge` 在 LOCK 内做机械 git merge，不经过 backend。
