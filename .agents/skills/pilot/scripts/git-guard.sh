...
case "$sub" in
  add)
    ...
    # HARD RULE (SKILL.md #3, v1.5.0): development happens in a per-Feature worktree, never in
    # the primary checkout. Staging is the chokepoint every commit must pass, so enforce here.
    # A linked worktree's --git-dir lives under <common>/worktrees/<name>; only in the primary
    # worktree do the two resolve to the same directory. Normalized via cd/pwd because rev-parse
    # may return relative paths (e.g. ".git") depending on CWD.
    if [ "${PILOT_ALLOW_PRIMARY:-0}" != "1" ]; then
      _gd="$(cd "$(git rev-parse --git-dir)" && pwd)"
      _gc="$(cd "$(git rev-parse --git-common-dir)" && pwd)"
      if [ "$_gd" = "$_gc" ]; then
        die "refusing to stage in the PRIMARY worktree — development happens in a per-Feature worktree (SKILL.md rule 3).
  Create/enter it first:  git worktree add ../<repo>-<Fid> -b <type>/<taskid>-<slug> <integration>
  (Deliberate, user-approved exception only: PILOT_ALLOW_PRIMARY=1)"
      fi
    fi
    for a in "$@"; do
      case "$a" in
...
' 2>/dev/null || true)"
          case "$why" in
            *"Branch not protected"*|*"Branch not found"*)
              # RULESET FALLBACK. GitHub has TWO protection mechanisms, and the classic endpoint
              # above only sees one of them: for a branch protected by a *ruleset* it answers
              # "Branch not protected", which is the opposite of the truth. Without this fallback
              # the rail refuses on a branch that really does require review, and its error tells
              # the operator to go protect the branch — work that is already done. (Observed on
              # iDoris-ai/Agent24: classic says not protected, /branches/main says protected:true,
              # /rules/branches/main lists three pull_request rules each requiring 1 approval.)
              #
              # Fail-closed is UNCHANGED. This only adds a second place to look for the same two
              # proofs; anything unreadable, absent or unenforced still falls through to the same
              # refusal below.
              #
              # Enforcement is verified EXPLICITLY rather than assumed. A ruleset may be `active`,
              # `evaluate` (report-only — rules appear to exist while enforcing nothing) or
              # `disabled`, and the per-branch rules endpoint does NOT carry that field. The docs
              # say it returns only active rules; this script's own standard is that unverified
              # protection is the same as none, so the ruleset list is fetched and only rules
              # belonging to an `active` ruleset are counted. If that list cannot be read, this
              # proves nothing and refuses.
              #
              # Both proofs still come from ONE captured payload ($rules), so — exactly as in the
              # classic path — the approval count and the stale-dismissal flag cannot disagree
              # with each other.
              rules="$(gh api "repos/$repo/rules/branches/$integration" 2>/dev/null || true)"
              rsets="$(gh api "repos/$repo/rulesets" 2>/dev/null || true)"
              rs_proof="$(printf '%s' "$rules" | PILOT_RSETS="$rsets" python3 -c '
import json, os, sys
try:
    rules = json.load(sys.stdin)
    rsets = json.loads(os.environ.get("PILOT_RSETS") or "[]")
except Exception:
    sys.exit(0)
if not isinstance(rules, list) or not isinstance(rsets, list):
    sys.exit(0)
active = {r.get("id") for r in rsets
...
for r in rules:
    if not isinstance(r, dict) or r.get("type") != "pull_request":
        continue
    # Only rules from a ruleset we CONFIRMED is active.
    if r.get("ruleset_id") not in active:
        continue
    p = r.get("parameters") or {}
    c = p.get("required_approving_review_count")
    # Every active ruleset applies at once, so the effective requirement is the
    # strictest one — and stale reviews are dismissed if ANY of them says so.
    if isinstance(c, bool) or not isinstance(c, int):
        continue
    if c > best:
        best = c
    if p.get("dismiss_stale_reviews_on_push"):
        dismiss = 1
if best >= 0:
    print("%d %d" % (best, dismiss))
' 2>/dev/null || true)"
              case "$rs_proof" in
                [0123456789]*" "[01])
                  approvals="${rs_proof% *}"
                  # Consumed by the stale-dismissal check below INSTEAD of re-parsing $prot,
                  # which on this path holds an error body rather than a protection object.
                  ruleset_dismiss="${rs_proof##* }"
                  echo "git-guard: '$integration' is protected by a RULESET (classic endpoint reports it unprotected)" >&2 ;;
                *)
                  die "--allow-trunk: branch '$integration' has no protection rule (classic endpoint said: $why; no active ruleset requiring review either) — refusing.
  ...
  Note: rulesets in 'evaluate' mode do NOT count — they report without enforcing." ;;
              esac ;;
            *)
  ...
  Protect the branch first; otherwise merging into trunk really is unreviewed self-merge."
      # Parsed from the SAME captured payload as the approval count — no second API call on either
      # path, so the two proofs cannot disagree with each other. Empty (unparseable / key absent)
      # falls through to the same refusal as an explicit false: unverified is treated as absent,
      # like every other proof on this path.
      if [ -n "${ruleset_dismiss:-}" ]; then
        # Came from the ruleset fallback, out of the same $rules payload that produced $approvals.
        dismiss="$ruleset_dismiss"
      else
...
' 2>/dev/null || true)"
  ...
  Wait for the verdict (see reference/review-contract.md). Never self-approve, never --admin."
    ...
    # ---- CI gate (FU-41). ---------------------------------------------------------------------
    # This script used to check WHO approved and WHERE it merges, and nothing about whether the code
    # passed. That gap merged aastar-sdk#347 with `Unit Tests=fail`: the operator had always looked
    # first, so the habit passed for a guarantee until the one time it did not.
    #
    # Read `status`, not `conclusion`. A running check has `conclusion: null`, and every naive
    # rendering of null — empty string, the literal "null", a `// "PENDING"` fallback that does not
    # fire on "" — makes an in-progress run look exactly like a passing one. That is not
    # hypothetical: the author of this gate hit it the same day, in a monitor that printed
    # RESULT=GREEN while two checks were still `in_progress`.
    #
    # And count the runs. ZERO checks and ZERO failing checks are the same reading of
    # "no failures found", while meaning opposite things — GitHub intermittently creates no run at
    # all for a sha, and `gh pr checks` then prints nothing, which looks like a clean bill.
    if [ -z "${GIT_GUARD_IGNORE_CHECKS:-}" ]; then
      head_sha="$(gh pr view "$n" --repo "$repo" --json headRefOid --jq .headRefOid 2>/dev/null || true)"
      [ -n "$head_sha" ] || die "cannot read PR #$n head sha — refusing to merge without reading CI"
      # Collapse to the LATEST run per check name. GitHub keeps every run for a sha, so a
      # re-run leaves the earlier conclusion in place forever; judging on "any failure
      # anywhere" then blocks a head whose checks are all green, permanently, with no way
      # forward but the override. Measured on SuperPaymaster#426: the first
      # `preflight-report` failed because no approval existed yet, the approval arrived,
      # the re-run passed -- and this gate still refused, so the merge used
      # GIT_GUARD_IGNORE_CHECKS on a head that was genuinely clean. Teaching an operator
      # that the override is how you merge a green PR is worse than the gap it closes.
      #
      # Latest-per-name is GitHub's own semantics for required contexts, so this agrees
      # with the merge box rather than inventing a rule. It does NOT weaken the three
      # gates below: a name whose latest run fails still blocks, a name whose latest run
      # is unfinished still blocks, and zero runs still blocks. Verified on synthetic
      # inputs -- stale-fail-then-pass ALLOWS, failure-as-latest REFUSES, in-progress
      # REFUSES, a different name failing REFUSES, zero runs REFUSES -- and on the real
      # #426 head, which went from 2 blocking rows to 0.
      #
      # ONE request, emitting both streams with an L|/S| prefix: two calls could disagree
      # if a run completed between them. Still `gh api --jq`, not a pipe to jq(1), so this
      # gains no dependency -- gh's built-in jq handles group_by/sort_by/last/IN (checked).
      #
      # Superseded failures are printed rather than dropped: "an earlier run of this check
      # failed" is worth seeing even when it no longer decides anything.
      crj="$(gh api "repos/$repo/commits/$head_sha/check-runs" \
             --jq '.check_runs as $a
                   | ($a|group_by(.name)|map(sort_by(.started_at,.id)|last)) as $latest
                   | (($latest[]|"L|\(.status)|\(.conclusion)|\(.name)"),
                      ($a[] | select((.id|IN($latest[].id)|not) and .conclusion=="failure")
                            | "S|\(.name) at \(.started_at)"))' 2>/dev/null || true)"
      ci="$(printf '%s\n' "$crj" | sed -n 's/^L|//p')"
      superseded="$(printf '%s\n' "$crj" | sed -n 's/^S|/  superseded failure (not the latest run of this check): /p')"
      if [ -n "$superseded" ]; then
        echo "git-guard: earlier runs of some checks failed and were superseded:" >&2
        printf '%s\n' "$superseded" >&2
      fi
      total="$(printf '%s' "$ci" | grep -c . || true)"
      if [ "${total:-0}" -eq 0 ]; then
        die "PR #$n head $head_sha has NO check runs at all — that is not 'nothing failed', it is
'nothing ran'. GitHub sometimes creates no run for a sha; close/reopen does not fix it, pushing a new
sha does. Re-trigger CI, or set GIT_GUARD_IGNORE_CHECKS=1 if you genuinely mean to merge untested."
      fi
      not_done="$(printf '%s\n' "$ci" | grep -v '^completed|' || true)"
      if [ -n "$not_done" ]; then
        die "PR #$n has check runs that have not finished:
$(printf '%s\n' "$not_done" | sed 's/^/  /')
Wait for them. An unfinished check is not a passing check."
      fi
      bad="$(printf '%s\n' "$ci" | grep -vE '^completed\|(success|neutral|skipped)\|' || true)"
      if [ -n "$bad" ]; then
        die "PR #$n has failing check runs:
$(printf '%s\n' "$bad" | sed 's/^/  /')
Refusing to merge. Use GIT_GUARD_IGNORE_CHECKS=1 only for a deliberate, explained exception."
      fi
      echo "git-guard: CI ok — $total check name(s), latest run of each completed and non-failing." >&2
    fi
