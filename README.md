...
- When the readiness check finds no planning documents, the host prepends a real, reviewed task to write them — not a pointer at a skill this loop cannot invoke
- When the readiness panel blocks on being on the trunk or a detached checkout, a name field and button next to that check run the same `git switch -c` the check names; refused once the project is armed, or for an unusable name, the trunk itself, or one already taken
...
```bash
dsh plugin --profile web add @jhfnetboy/dsh-devloop@0.6.9
```
